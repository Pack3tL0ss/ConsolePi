...
# Don't think this was ever used (from common file)
# def get_local(log):
...
def check_reachable(ip, port, timeout=TIMEOUT):
    # if url is passed make sure we can get out first otherwise dns resolution failure causes longer delay
    if '.com' in ip:
        test_set = ['8.8.8.8', ip]
        timeout += 3
    else:
...
class ConsolePi_Log:
    ...
    def __init__(self, debug=None, log_file='/var/log/ConsolePi/cloud.log', do_print=True):
