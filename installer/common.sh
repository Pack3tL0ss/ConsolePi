...
get_pi_info() {
    ...
    git_rem=$(pushd /etc/ConsolePi >/dev/null && git remote -v | head -1 | cut -d '(' -f-1 ; popd >/dev/null)
    [[ $(echo $git_rem | awk '{print $2}') != $consolepi_source ]] && logit "Using alternative repo: ${_green}$git_rem${_norm}"
    # cat /etc/os-release
