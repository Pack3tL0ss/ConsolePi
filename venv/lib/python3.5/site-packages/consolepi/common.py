...
class ConsolePi_Log:
    ...
    def __init__(self, debug=None, log_file=LOCAL_CLOUD_FILE, do_print=None):
...
def update_local_cloud_file(local_cloud_file=LOCAL_CLOUD_FILE, remote_consoles=None):
    ...
    if remote_consoles is not None and len(remote_consoles) > 0:
        ...
        if current_remotes is not None:
            for _ in current_remotes:
                ...
                elif 'rem_ip' in current_remotes[_] and 'rem_ip' not in remote_consoles[_]:
...
def get_local_cloud_file(local_cloud_file=LOCAL_CLOUD_FILE):
    data = {}
    if os.path.isfile(local_cloud_file):
...
# Not Used yet, just trying to clean up, better organize the code
def get_config_all():
     ...
     def __init__(self):
        ...
        for key, value in config.items():
            if type(value) == str:
                ...
                exec('self.{} = {}'.format(key, value))
        self.cpi_log = ConsolePi_Log(debug=self.debug, do_print=False)
        self.log = self.cpi_log.log
        self.hostname = socket.gethostname()
        self.adapters = get_local(cpi_log=self.cpi_log, do_print=False)
        self.interfaces = get_if_ips(self.log)
        self.local = {self.hostname: {'adapters': self.adapters, 'interfaces': self.interfaces, 'user': 'pi'}}
        self.remotes = get_local_cloud_file()
