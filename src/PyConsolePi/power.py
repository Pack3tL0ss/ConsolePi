...
class Outlets:
    def __init__(
        self, power_file="/etc/ConsolePi/power.json", log=getLogger("ConsolePi-PWR")
    ):
        ...
        self.dli_exists = (
            self.tasmota_exists
        ) = self.gpio_exists = self.linked_exists = False
    ...
    def do_tasmota_cmd(self, address, command=None):
        ...
        def tasmota_req(*args, **kwargs):
            ...
            try:
                response = requests.request(
                    "GET", url, headers=headers, params=querystring, timeout=3
                )
        ...
        if command is not None:
            ...
            if command in ["ON", "OFF", "TOGGLE"]:
                ...
                command == "CYCLE"
            ):  # Power off if cycle is command, powered back on below
    ...
    def pwr_get_outlets_from_file(self):
        ...
        if path.isfile(self.power_file):
            ...
            for k in outlet_data["linked"]:
                if (
                    "linked_devs" in outlet_data["linked"][k]
                    and outlet_data["linked"][k]["linked_devs"]
                ):
                    outlet_data["linked"][k]["linked_devs"] = self.pwr_format_dev(
                        outlet_data["linked"][k]["linked_devs"]
                    )
                    self.linked_exists = True
                exec(
                    "self.{}_exists = True".format(
                        outlet_data["linked"][k]["type"].lower()
                    )
                )
        else:
            outlet_data = None
    ...
    def pwr_start_update_threads(self, upd_linked=False, failures={}, t_name="init"):
        ...
        outlet_data = (
            self.outlet_data["linked"] if "linked" in self.outlet_data else None
        )
        if not failures:
        ...
        if outlet_data is not None:
            for k in outlet_data:
                ...
                if not found:
                    threading.Thread(
                        target=self.pwr_get_outlets,
                        args=[{k: outlet_data[k]}],
                        kwargs=kwargs,
                        name=t_name + "_pwr_" + k,
                    ).start()
    ...
    def pwr_get_outlets(self, outlet_data=None, upd_linked=False, failures={}):
        ...
        for k in outlet_data:
            ...
            if outlet["type"].upper() == "GPIO":
                noff = (
                    True if "noff" not in outlet else outlet["noff"]
                )  # default normally off to True if not provided
                GPIO.setup(
                    outlet["address"], GPIO.OUT
                )  # pylint: disable=maybe-no-member
                outlet_data[k]["is_on"] = (
                    bool(GPIO.input(outlet["address"]))
                    if noff
                ...
                if response not in [0, 1, True, False]:
                    ...
                    failures[k][
                        "error"
                    ] = "[PWR-TASMOTA {}:{}] Returned Error - Removed".format(
                        k, failures[k]["address"]
                    )
            elif outlet["type"].lower() == "dli":
                if TIMING:
                    dbg_line = "------------------------ // NOW PROCESSING {} \\\\ ------------------------".format(
                        k
                    )
                    print("\n{}".format("=" * len(dbg_line)))
                    print(
                        "{}\n{}\n{}".format(
                            dbg_line, outlet_data[k], "-" * len(dbg_line)
                        )
                    )
                ...
                for _ in ["address", "username", "password"]:
                    if _ not in outlet or outlet[_] is None:
                        ...
                        failures[k][
                            "error"
                        ] = "[PWR-DLI {}] {} missing from {} configuration - skipping".format(
                            k, _, failures[k]["address"]
                        )
                        # Log here, delete item from dict? TODO
                        break

                if all_good:
                    (this_dli, _update) = self.load_dli(
                        outlet["address"], outlet["username"], outlet["password"]
                    )
                    if this_dli is None or this_dli.dli is None:
                        failures[k] = outlet_data[k]
                        failures[k][
                            "error"
                        ] = "[PWR-DLI {}] {} Unreachable - Removed".format(
                            k, failures[k]["address"]
                        )
                    else:
                        if TIMING:
                            xstart = time.time()
                            print(
                                "this_dli.outlets: {} {}".format(
                                    this_dli.outlets, "update" if _update else "init"
                                )
                            )
                        ...
                        if _update:
                            if not upd_linked:
                                dli_power[
                                    outlet["address"]
                                ] = this_dli.get_dli_outlets()
                                if not dli_power[outlet["address"]]:
                                    all_good = False
                                if (
                                    all_good
                                    and "linked_ports" in outlet
                                    and outlet["linked_ports"] is not None
                                ):
                                ...
                                if "linked_ports" in outlet and outlet["linked_ports"]:
                                    ...
                                    if k in outlet_data:
                                        ...
                                        self.log.error(
                                            "[PWR GET_OUTLETS] {} appears to be unreachable".format(
                                                k
                                            )
                                        )
                                    ...
                                    if (
                                        isinstance(outlet["is_on"], dict)
                                        and not outlet["is_on"]
                                    ):
                                        all_good = False
                                    # update dli_power for the refreshed ports
                                    else:
                                        for _ in outlet["is_on"]:
                                            dli_power[outlet["address"]][_] = outlet[
                                                "is_on"
                                            ][_]
                        ...
                        if TIMING:
                            print(
                                "[TIMING] this_dli.outlets: {}".format(
                                    time.time() - xstart
                                )
                            )  # TIMING
    ...
    def pwr_toggle(
        ...
        if isinstance(
            desired_state, str
        ):  # menu should be passing in True/False no on off now. can remove once that's verified
            desired_state = False if desired_state.lower() == "off" else True
            print(
                "\ndev_note: pwr_toggle passed str not bool for desired_state check calling function {}".format(
                    desired_state
                )
            )
            time.sleep(5)
        ...
        # -- // Toggle GPIO port \\ --
            ...
            if desired_state is None:
                cur_state = (
                    bool(GPIO.input(gpio)) if noff else not bool(GPIO.input(gpio))
                )  # pylint: disable=maybe-no-member
        ...
        # -- // Toggle TASMOTA port \\ --
            ...
            raise Exception(
                "pwr_toggle: Invalid type ({}) or no name provided".format(pwr_type)
            )
    ...
    def pwr_rename(self, type, address, name=None, port=None):
        ...
        if type.lower() == "dli":
            ...
            raise Exception(
                "pwr_rename: Invalid type ({}) or no name provided".format(type)
            )
    ...
    def pwr_all(self, outlets=None, action="toggle", desired_state=None):
        ...
        if action == "toggle" and desired_state is None:
            return (
                "Error: desired final state must be provided"  # should never hit this
            )
        ...
        for grp in outlets:
            ...
            if action == "toggle":
                ...
                if not outlet["type"] == "dli" or (
                    "linked_ports" in outlet and outlet["linked_ports"]
                ):
                    # threading.Thread(target=responses.append, args=(self.pwr_toggle(outlet['type'],
                    # outlet['address'], desired_state=desired_state,
                    responses.append(
                        self.pwr_toggle(
                            outlet["type"],
                            outlet["address"],
                            desired_state=desired_state,
                            port=outlet["linked_ports"]
                            if outlet["type"] == "dli" and "linked_ports" in outlet
                            else None,
                            noff=noff,
                            noconfirm=True,
                        )
    ...
    def pwr_format_dev(self, dev, with_path=True):
        ...
        for d in dev:
            d = d.replace(
                "/host/", ""
            )  # TODO get config.ssh_hosts to determine which way to go...
            if with_path:
                if "/dev/" not in d:
                    d = "/dev/{}".format(d)
            else:
